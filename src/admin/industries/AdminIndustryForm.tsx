...
import type { Industry, IndustryUseCase } from "../../types/industry";

interface Props {
  industry: Industry | null;
  onClose: () => void;
  onSave: (data: Partial<Industry>, id?: number) => Promise<void>;
  saving: boolean;
}
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 rounded-xl border border-black/[0.10] bg-black/[0.02] p-2.5 dark:border-white/[0.10] dark:bg-white/[0.03]">
        {values.map((v) => (
          <span key={v} className="flex items-center gap-1.5 rounded-lg bg-[#2f8fe6]/10 px-2.5 py-1 text-xs font-medium text-[#2f8fe6] dark:text-[#58adff]">
            {v}
...
export default function AdminIndustryForm({ industry, onClose, onSave, saving }: Props) {
  ...
  const addUseCase = () => set("use_cases", [...form.use_cases, { title: "", desc: "" }]);

  const updateUseCase = (index: number, key: keyof IndustryUseCase, val: string) =>
    set("use_cases", form.use_cases.map((uc, i) => (i === index ? { ...uc, [key]: val } : uc)));
        ...
        <div className="flex items-center justify-between border-b border-black/[0.07] px-6 py-4 dark:border-white/[0.08]">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#2f8fe6]">
              {industry ? "Edit" : "New"} Industry
            </p>
            <h2 className="mt-0.5 text-base font-semibold tracking-[-0.03em]">
              {industry ? `Editing: ${industry.title}` : "Create a new industry"}
            </h2>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="flex flex-col overflow-hidden">
          <div className="flex-1 space-y-4 overflow-y-auto px-6 py-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-black/60 dark:text-white/55">Icon name</label>
                <input
                  value={form.icon}
                  ...
                  className={inputCls}
                />
                <p className="mt-1 text-[11px] text-black/35 dark:text-white/30">Lucide icon name (PascalCase)</p>
              </div>
            </div>
            ...
            {/* Full description */}
            <div>
              <label className="mb-1.5 block text-xs font-semibold text-black/60 dark:text-white/55">Full Description</label>
              <textarea
                rows={4}
            ...
            <TagInput
              label="Challenges (press Enter to add)"
              values={form.challenges}
              onChange={(v) => set("challenges", v)}
            />
            <TagInput
              label="Solutions / How we help (press Enter to add)"
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                <label className="text-xs font-semibold text-black/60 dark:text-white/55">Use Cases</label>
                <button
                  type="button"
                  onClick={addUseCase}
              ...
              <div className="space-y-3">
                {form.use_cases.map((uc, i) => (
                  <div key={i} className="relative rounded-xl border border-black/[0.08] bg-black/[0.02] p-4 dark:border-white/[0.08] dark:bg-white/[0.02]">
                    <button
                      type="button"
                    ...
                    <div className="space-y-2.5">
                      <input
                        ...
                        placeholder="Title (e.g. Patient Management)"
                        className={inputCls}
                      />
                      <textarea
                        rows={2}
            ...
            <div className="flex items-center justify-between rounded-xl border border-black/[0.08] bg-black/[0.02] px-4 py-3 dark:border-white/[0.08] dark:bg-white/[0.03]">
              <div>
                ...
                <p className="text-[11px] text-black/40 dark:text-white/35">Visible on the public website</p>
              </div>
              <button
                type="button"
