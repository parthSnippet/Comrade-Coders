...
export default function AdminServicesPage() {
  const { data, loading, error, request } = useAPI<Service[]>();
  const {
    loading: saving,
    error: mutateError,
    request: mutate,
  ...
  const load = () =>
    request(apiConfig.api.endpoints.services, "GET", undefined, {
  ...
  const handleSave = async (
    ...
    /*
     * youtube_url belongs to ServiceVideo, not Service.
     * Keep it temporarily and remove it from the Service payload.
     */
    const youtubeUrl = formData.youtube_url?.trim() || "";

    const servicePayload = { ...formData };
    delete servicePayload.youtube_url;

    const endpoint = id
      ? `${apiConfig.api.endpoints.services}${id}/`
      : apiConfig.api.endpoints.services;

    const method = id ? "PUT" : "POST";

    /*
     * 1. Save the actual Service first.
     */
    ...
    if (!res) {
      showToast(mutateError || "Something went wrong.", "error");
      return;
    }

    const savedService = res as ServiceWithVideos;

    /*
     * 2. Handle Service Video separately.
     *
     * For an existing service, fetch the latest service object
     * so we can check whether a video already exists.
     */
    let existingVideo: ServiceVideo | undefined;

    if (savedService.id) {
      const latestService = await mutate(
        `${apiConfig.api.endpoints.services}${savedService.id}/`,
        "GET",
        undefined,
        {
          headers: getAuthHeader(),
      ...
      if (latestService) {
        existingVideo = (
          latestService as ServiceWithVideos
        ).videos?.find(
          (video) =>
    ...
    /*
     * 3. If a YouTube URL is provided:
     *    - PATCH existing video
     *    - otherwise POST a new video
     */
    if (youtubeUrl) {
      const videoPayload = {
      ...
      const videoEndpoint = existingVideo
        ? `${apiConfig.api.endpoints.serviceVideos}${existingVideo.id}/`
        : apiConfig.api.endpoints.serviceVideos;

      const videoMethod = existingVideo ? "PATCH" : "POST";

      const videoRes = await mutate(
        videoEndpoint,
        videoMethod,
        videoPayload,
        {
      ...
      if (!videoRes) {
        showToast(
          "Service saved, but video could not be saved.",
          "error"
        );
        return;
      }
  ...
  const handleToggleActive = async (service: Service) => {
    await mutate(
      `${apiConfig.api.endpoints.services}${service.id}/`,
      "PATCH",
  ...
  const handleDelete = async () => {
    ...
    await mutate(
      `${apiConfig.api.endpoints.services}${deleteId}/`,
      "DELETE",
      undefined,
      { headers: getAuthHeader() }
    );
  ...
  return (
    <div className="space-y-6">
      ...
      {toast && (
        <div
          className={`fixed bottom-6 right-6 z-[60] rounded-xl border px-5 py-3 text-sm font-medium shadow-lg transition-all ${
            toast.type === "success"
      ...
      <div className="rounded-2xl border border-black/[0.07] bg-white dark:border-white/[0.08] dark:bg-white/[0.04]">
        ...
        {error && !loading && (
          <div className="px-5 py-8 text-center text-sm text-red-500">
            Failed to load services. Check your API connection.
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-[3px]">
          <div className="w-full max-w-sm rounded-2xl border border-black/[0.10] bg-white p-6 shadow-2xl dark:border-white/[0.10] dark:bg-[#0d1219]">
            ...
            <p className="mt-1.5 text-sm text-black/45 dark:text-white/40">
              This action cannot be undone. The service will
